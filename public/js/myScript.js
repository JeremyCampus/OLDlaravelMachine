...
$( ".choix" ).click(function() {
    ...
    nomBoissonSelected = $( this ).text();
    monPrix = $( this ).attr( 'href' );
    let selected = $( ".choix" ).hasClass( "selected" );
    $("#monnayeur p").html(nomBoissonSelected+" Selectionné");
    selectDrink(selected, boissonSelected);
});

function validerFormulaire2(f) {
    alert("test");
    var prixOK = verifPseudo(f.prix);
    var idOK = verifMail(f.id);
    var sucrePourPHPOK = verifAge(f.sucrePourPHP);

    if(!prixOK==""  && !idOK=="" && !sucrePourPHPOK=="")
        return false;
    else
    {
...
function selectDrink(doSelect, drink){
    ...
    var monPrix = document.getElementById("prix");
    var lePrix = $("#"+drink).attr('href');
    if($('#' + drink).hasClass("selected") || drink === "reset")//SI la boisson a deja la classe "selected"
    {
        $('.choix').removeClass("selected");     //Enlever les choix "selected"
        monId.setAttribute("value", '');
        monPrix.setAttribute("value", "");
    }else{//----------------------------------------------//SINON
        ...
        monId.setAttribute("value", drink);
        monPrix.setAttribute("value", lePrix);
    }
}
...
function majCredit()
{
    ...
    for(i = 0 ; i < 6; i++){
     ...
     $( "#monnaieMise" ).attr( 'value',  monnaie);
}
...
//     $.ajax({url: "eau.php", success: function(result){
//         let doseEau =parseInt(result);
...
//     $.ajax({url: "cafe.php", success: function(result){
//         let doseCoffee =parseInt(result);
